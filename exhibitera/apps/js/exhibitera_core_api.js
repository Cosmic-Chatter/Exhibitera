...
export function writeRawText (name, text, mode = 'a', local = true) {
  // Send text to Exhibitera to be saved to disk
  ...
  // If `local` is false, send the data to Hub if it is in use (this allows data
  // to be aggregated between multiple app instances)
  ...
  if (window.parent.exCommon.standalone || local === true) {
    window.parent.exCommon.makeHelperRequest(options)
  } else window.parent.exCommon.makeServerRequest(options)
}

export async function getRawText (name, local = true) {
  ...
  if (window.parent.exCommon.standalone || local === true) {
    result = await window.parent.exCommon.makeHelperRequest(options)
  } else result = await window.parent.exCommon.makeServerRequest(options)
